...
const https_ka      = require("https");  // for keep-alive ping

const { handleMessage }           = require("./lumeo_core");
const { getMorningBriefing, getStatusContent, LUMEO_VERSION, getDevGreeting } = require("./personality");
const { isBanned, checkRateLimit, getHistory, cacheUser }     = require("./lumeo_users");
const { setStatusSock, handleStatusMessage, postStatus, postImageStatus, generateStatusContent } = require("./lumeo_status");
...
function startKeepAlive() {
  ...
  setInterval(async () => {
    try {
      ...
      const pingMod = RENDER_URL.startsWith("https") ? https_ka : http;
      pingMod.get({ hostname: u.hostname, path: "/", port: u.port || (RENDER_URL.startsWith("https") ? 443 : 80) }, r => {
...
function setupCron(sock) {
  ...
  // Status update — 7 AM text, 12 PM image, 6 PM text (WAT)
  cron.schedule("0 7,18 * * *", async () => {
    try {
      const sc = await generateStatusContent();
      await postStatus(sc);
    } catch {}
  }, { timezone: "Africa/Lagos" });

  // Midday: post AI-generated promotional image to status
  cron.schedule("0 12 * * *", async () => {
    try {
      const { generateImage } = require("./ai");
      const prompts = [
        "EMEMZYVISUALS DIGITALS digital agency logo glowing tech professional",
        "AI technology neural network abstract blue green vibrant modern",
        "Nigerian developer coding laptop screen glowing innovation",
        "ClaudGPT AI assistant interface modern sleek professional",
        "digital marketing social media campaign vibrant energetic",
      ];
      const prompt  = prompts[Math.floor(Math.random() * prompts.length)];
      const imgBuf  = await generateImage(prompt);
      if (imgBuf) {
        const caption = await generateStatusContent("EMEMZYVISUALS DIGITALS");
        await postImageStatus(imgBuf, caption);
      }
    } catch (e) { console.error("[Cron] Image status error:", e.message); }
  }, { timezone: "Africa/Lagos" });
...
let _401Count        = 0;      // track consecutive 401s for backoff
let _lastConnect     = 0;      // prevent rapid reconnects
...
async function connect() {
  ...
  sock.ev.on("connection.update", async (update) => {
    ...
    if (connection === "open") {
      _401Count = 0;  // Reset backoff counter on successful connect
    ...
    if (connection === "close") {
      ...
      if (shouldReconnect) {
        ...
        // 401 = logged out — exponential backoff
        _401Count++;
        const backoffMs = Math.min(15000 * Math.pow(2, _401Count - 1), 300000);
        console.log(`[Bot] Logged out (401) #${_401Count} — clearing session, reconnecting in ${Math.round(backoffMs/1000)}s...`);
        try {
        ...
        _pairingDone = false;
        _reconnectTimer = setTimeout(connect, backoffMs);
  ...
  sock.ev.on("messages.upsert", async ({ messages, type }) => {
    ...
    for (const msg of messages) {
      try {
        ...
        // Mark message as read immediately (shows blue ticks to sender)
  ...
  // ── Poll vote handler ─────────────────────────────────────────────────────
  // When user taps a poll option, WhatsApp sends a messages.update event
  sock.ev.on("messages.update", async (updates) => {
    for (const update of updates) {
      try {
        ...
        // Decode the selected option name
        const selectedVotes = pollUpdate.vote?.selectedOptions || [];
        if (selectedVotes.length === 0) continue;

        // Get the original poll message to find the option text
        const pollMsgKey = update.key;
        const phone      = normalizePhone(voterJid);
        const isDev      = isDeveloper(voterJid);
        const jid        = voterJid.includes("@g.us") ? voterJid : voterJid;
        const replyJid   = voterJid.endsWith("@s.whatsapp.net") ? voterJid : voterJid;

        // Selected option comes as a SHA256 hash — we need the original poll to decode
        // Best approach: get from poll message, or just pass the hash as text to handler
        // Since we control poll content we know what options map to
        const optionName = selectedVotes[0]?.name || "";
        if (!optionName) continue;

        console.log("[Poll] " + phone + " voted: " + optionName);

        // Route the poll selection through the normal message handler
        await handleMessage({
          text: optionName, phone, pushName: "",
          sock, jid: replyJid, senderJid: voterJid,
          isGroup: false, isDev,
          hasImage: false, hasAudio: false, hasVideo: false,
          hasSticker: false, hasDocument: false,
          groupJids: [],
        });
      } catch (e) {
