...
function ytdlp() {
  try {
    ...
    // Try to use system yt-dlp first (installed via pip3 on Render)
    // This avoids the "Syntax error: newline unexpected" from the bundled binary
    const { execSync } = require("child_process");
    try {
      const sysPath = execSync("which yt-dlp 2>/dev/null || echo ''", { encoding: "utf8" }).trim();
      if (sysPath && sysPath.length > 3) {
        console.log("[yt-dlp] Using system binary:", sysPath);
        return require("youtube-dl-exec").create(sysPath);
      }
    } catch {}
    return ydl;
  } catch { return null; }
...
async function ytdlCoreDownload(url, audioOnly, ts) {
  try {
    ...
    const outPath = path.join(TMP, `lumeo_${ts}${audioOnly ? ".mp3" : ".mp4"}`);
    ...
    await new Promise((resolve, reject) => {
      try {
        const opts = audioOnly
          ...
          : { quality: "highest", filter: f => f.container === "mp4" && f.hasAudio && f.hasVideo };
        ...
        stream.on("error", (e) => {
          console.log("[ytdl-core] Stream error:", e.message?.slice(0, 80));
          try { ws.destroy(); } catch {}
          // If 429 or rate limit, don't reject - just resolve null
          resolve(null);
        });

        ws.on("finish", resolve);
        ws.on("error", (e) => {
          console.log("[ytdl-core] Write error:", e.message?.slice(0, 80));
          resolve(null);
        });

        stream.pipe(ws);
      } catch (e) {
        console.log("[ytdl-core] Init error:", e.message?.slice(0, 80));
        resolve(null);
      }
    });
    ...
    if (fs.existsSync(outPath) && fs.statSync(outPath).size > 1000) {
      ...
      console.log(`[ytdl-core] ✅ ${title.slice(0,50)} — ${(size/1024/1024).toFixed(1)}MB`);
