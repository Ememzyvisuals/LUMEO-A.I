...
const { getLumeoPrompt }           = require("./personality");
...
async function humanTyping(sock, jid, len = 100) {
  try {
    ...
    await new Promise(r => setTimeout(r, Math.min(len * 20 + Math.random() * 500, 4000)));
    ...
    setImmediate(async () => {
      try {
        ...
        if (result.videoBuf) {
          ...
          await sock.sendMessage(replyJid, { audio: result.audioBuf, mimetype: "audio/mpeg", fileName: `${searchQ.slice(0, 40)}.mp3`, caption: `🎵 ${searchQ}` });
          // Send lyrics link
          ...
          if (GENIUS) {
            try {
              ...
              const gData = await new Promise(res => {
                const req = https.get({ hostname: "api.genius.com", path: `/search?q=${encodeURIComponent(searchQ)}`, headers: { "Authorization": `Bearer ${GENIUS}` }, timeout: 8000 }, r => {
            ...
            await sock.sendMessage(replyJid, { text: `📝 Lyrics: https://www.google.com/search?q=${encodeURIComponent(searchQ + " lyrics")}`, mentions: [] });
          }
        }
