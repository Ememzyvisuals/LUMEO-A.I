...
// Language is always English unless user explicitly requests another language
// Lumeo detects context but RESPONDS in English by default
...
async function sendSmart(sock, jid, text, phone, mentions = []) {
  if (!text || !jid) return;
  // If JID is LID format (@lid), skip - can't send to LID directly
  // Message was received on this JID so Baileys should handle it
  try {
    await sock.sendMessage(jid, { text, mentions });
  } catch (e) {
    console.error("[Send] Failed to", jid.split("@")[0], ":", e.message.slice(0, 80));
  }
}
...
async function handleMessage(params) {
  ...
  const sysPrompt = getLumeoPrompt({
    ...
    language: "english",  // Always English — change only if user explicitly asks
    currentTime: nowWAT,
  ...
  // ══════════════════════════════════════════════════════════════════════════
  // ROUTE 5: ADMIN COMMANDS (dev only, no AI classification needed)
  // ══════════════════════════════════════════════════════════════════════════
  if (isDev) {
...
• send email to email@x.com about [project]

` +
        `*Status:*
• post status [text]
• post status about [topic]

` +
        `*Users:*
• list users — show all known users
• clear memory — wipe my memory
• voice note mode / text mode
...
• ping — check if alive`
    ...
    if (t === "ping" || t === ".ping") {
      await send(`🟢 *Lumeo v${LUMEO_VERSION} is alive!*
...
Memory: ${(process.memoryUsage().rss / 1024 / 1024).toFixed(0)}MB`);
    ...
    if (t === "list users" || t === "show users" || t === ".users") {
      const { getAllCachedPhones } = require("./lumeo_users");
      ...
      if (all.length === 0) { await send("No users cached yet. Wait for someone to message Lumeo first!"); return; }
      const lines = all.slice(0, 50).map((p, i) => {
        const u = dbUsers.find(x => x.phone === p);
        return (i + 1) + ". +" + p + (u?.name ? " — " + u.name : "") + (u?.banned ? " 🚫" : "");
      });
      const userList = "Known Users (" + all.length + "):\n\n" + lines.join("\n") + (all.length > 50 ? "\n...and " + (all.length - 50) + " more" : "");
      await send(userList);
      return;
    }
    ...
    if (t === "clear memory" || t === "wipe memory" || t === ".clearmem") {
      ...
      await send("✅ Your memory has been cleared boss!");
      return;
    }
  }
...
${hit.url}`,
  ...
  if (intent.type === "status_post") {
    ...
    // Check if user wants image/video status
    if (/generate.*image.*status|image.*status|post.*image.*status|status.*image/i.test(rawStatus)) {
      await sock.sendMessage(replyJid, { text: "Generating image for status... 🎨", mentions });
      const imgBuf = await generateImage(rawStatus.replace(/generate|post|on.*status|status/gi,"").trim() || "EMEMZYVISUALS DIGITALS promotional image, professional, vibrant");
      if (imgBuf) {
        const caption = await generateStatusContent("EMEMZYVISUALS DIGITALS");
        const ok = await postImageStatus(imgBuf, caption);
        await send(ok ? "✅ Image posted to status!" : "❌ Image status failed — ensure contacts are loaded.");
      } else { await send("❌ Image generation failed. Try again!"); }
      return;
    }

    if (/generate.*video.*status|video.*status|post.*video.*status|status.*video|market.*video.*status/i.test(rawStatus)) {
      await sock.sendMessage(replyJid, { text: "Generating video for status... 🎬", mentions });
      try {
        const { generateVideo } = require("./lumeo_video");
        const vr = await generateVideo(rawStatus.replace(/generate|post|on.*status|status|market/gi,"").trim() || "EMEMZYVISUALS DIGITALS services promotion");
    ...
    await send(ok ? "✅ Status posted!\n\n_\"" + statusContent.slice(0, 100) + "...\"_" : "❌ Status post failed. Run \"list users\" to check if contacts are loaded.");
    return;
  }
  ...
  const chatSys = sysPrompt +
    (history.length > 2 ? "\n\nThis is an ONGOING conversation. Reply DIRECTLY — no greetings, no 'How can I help?'. Just respond naturally." : "");
