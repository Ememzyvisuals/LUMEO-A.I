...
function detectLanguage(text) {
  ...
  // Pidgin markers
  if (/\b(omo|abeg|wahala|wetin|shey|dey|na so|e be|no be|sabi|oya|guy|bro|nah|dem|una|nor|abi|comot|chop|gist|nau|make i|how far|wey|deh|sef|chai|haba|toh|bros)\b/.test(t)) return "pidgin";
  // Yoruba
  if (/\b(ẹ|ọ|se|bawo|pẹlẹ|odabo|ekaro|ekaabo|ẹnikan|ile|omi|ọrun)\b/.test(t)) return "yoruba";
  // Hausa
  if (/\b(sannu|yaya|lafiya|marhaba|nagode|yauwa|kuma|don allah)\b/.test(t)) return "hausa";
  // Igbo
  if (/\b(nnọọ|ndeewo|daalu|kedu|ọ dị mma|nna|nne|biko)\b/.test(t)) return "igbo";
  // French
  if (/\b(bonjour|salut|merci|oui|non|je|tu|vous|est-ce|comment|pourquoi|madame|monsieur)\b/.test(t)) return "french";
  return "english";
}
...
async function handleMessage(params) {
  ...
  const detectedLang = text ? detectLanguage(text) : (userRec?.language || "english");
