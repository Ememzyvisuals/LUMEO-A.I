...
const { sendWelcomeMenu, sendSuggestions, budgetHelper, jobHelper, businessHelper, studyHelper, adviceHelper, healthHelper, scamChecker, detectProblemType, pollOptionToText } = require("./lumeo_interactive");
...
const _seenUsers   = new Set();   // users who received welcome menu
...
async function sendSmart(sock, jid, text, mentions) {
  if (!text || !jid) return;
  // Only pass mentions in group chats — DMs reject mentionedJid
  const isGroupJid  = jid.endsWith("@g.us") || jid.includes("@g.us");
  const safeMentions = isGroupJid ? (Array.isArray(mentions) ? mentions : []) : [];
  try {
    const msg = safeMentions.length > 0 ? { text, mentions: safeMentions } : { text };
    await sock.sendMessage(jid, msg);
  } catch (e) {
    console.error("[Send] Failed:", jid?.split("@")[0], e.message?.slice(0, 80));
  }
}
...
async function handleMessage(params) {
  ...
  // ── First message → welcome menu for new DM users ────────────────────────
  if (!isGroup && text && !_seenUsers.has(phone)) {
    _seenUsers.add(phone);
    if (/^(hi|hello|hey|heyy|yo|sup|xup|good|morning|afternoon|evening|start|begin|menu|help|lumeo|what)\b/i.test(text.trim())) {
    // Welcome menu — wrapped in try/catch so failure doesn't block reply
    try { await sendWelcomeMenu(sock, replyJid, pushName); } catch {}
    addMemory(phone, 'user', text).catch(() => {});
    return;  // Menu sent — user taps option to continue
    }
  }

  // ── Save incoming ───────────────────────────────────────────────────────────
  const memEntry = text || (hasAudio ? "[Voice note]" : hasDocument ? "[File: " + (documentName || "doc") + "]" : hasSticker ? "[Sticker]" : null);
  if (memEntry && !hasImage) addMemory(phone, "user", memEntry).catch(() => {});  // fire-and-forget

  // ── Load history + build prompt ─────────────────────────────────────────────
  const history   = await getHistory(phone, 40);
  const userRec   = await Promise.race([
    getUser(phone),
  ...
  const saveReply = (r) => { if (r) addMemory(phone, "assistant", String(r).slice(0, 600)).catch(() => {}); };  // non-blocking
  ...
  // ── Poll vote interpretation (map poll option → action) ─────────────────────
  // When user taps a poll option it arrives as a regular text message
  if (/^(download music|play music|music|play another song|more afrobeats)/i.test(text.trim())) {
    return handleMessage({ ...params, text: "Play a trending Afrobeats song" });
  }
  if (/^(download video|video)/i.test(text.trim())) {
    return handleMessage({ ...params, text: "I want to download a video" });
  }
  if (/^(generate image|image|generate another)/i.test(text.trim())) {
    return handleMessage({ ...params, text: "Generate an image of something beautiful and vibrant" });
  }
  if (/^(create pdf|make pdf|pdf)/i.test(text.trim())) {
    return handleMessage({ ...params, text: "Create a professional PDF document" });
  }
  if (/^(post status|status)/i.test(text.trim()) && isDev) {
    return handleMessage({ ...params, text: "post status" });
  }
  if (/^(write my cv|cv)/i.test(text.trim())) {
    return handleMessage({ ...params, text: "Help me build a professional CV" });
  }
  if (/^(budget calculator|budget|expense tracker)/i.test(text.trim())) {
    await send("Tell me your income and expenses and I'll break it down for you. Example:\n\n_Income: ₦150,000\nRent: ₦40,000\nFood: ₦25,000\nTransport: ₦15,000\nData: ₦5,000_");
    return;
  }
  if (/^(whatsapp screenshot|screenshot)/i.test(text.trim())) {
    await send("Send me the conversation like this:\n\nJohn: Hey!\nMe: What's up?\nJohn: Nothing much\n\nI'll generate a realistic WhatsApp screenshot for you 📸");
    return;
  }
  if (/^(back to menu|menu|what can you do|help)/i.test(text.trim())) {
    await sendWelcomeMenu(sock, replyJid, pushName);
    return;
  }
  if (/^(solve a problem|solve problem|problem)/i.test(text.trim())) {
    const { sendPoll } = require("./lumeo_interactive");
    await sendPoll(sock, replyJid, "What type of problem?", [
      "Budget & expenses",
      "Job & career help",
      "Business idea",
      "Homework & study",
  ...
  if (problemType) {
    await humanTyping(sock, replyJid, 100);
    let problemReply = null;
    if (problemType === "budget")   problemReply = await budgetHelper(sock, replyJid, text);
    if (problemType === "job")      { await jobHelper(sock, replyJid, text); return; }
    if (problemType === "business") problemReply = await businessHelper(text);
    if (problemType === "study")    problemReply = await studyHelper(text);
    if (problemType === "advice")   problemReply = await adviceHelper(text);
    if (problemType === "health")   problemReply = await healthHelper(text);
    if (problemType === "scam")     problemReply = await scamChecker(text);
    if (problemReply) {
      await sendSmart(sock, replyJid, clean(problemReply), mentions);
      await saveReply(problemReply);
      await sendSuggestions(sock, replyJid, text);
      return;
    }
  }
