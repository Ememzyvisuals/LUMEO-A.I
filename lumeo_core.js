...
const { startCompetition, lumeoRespond, judgeRound, nextRound, endCompetition, getCompetition, isCompetitionActive } = require("./lumeo_competition");
const { postStatus, postImageStatus, postVideoStatus, generateStatusContent } = require("./lumeo_status");
...
async function classifyIntent(text, history) {
  ...
  const prompt =
    ...
    "- status_post: post a WhatsApp status update (text, image, or video)\n" +
...
async function handleMessage(params) {
  ...
  // ── AI Competition ────────────────────────────────────────────────────────
  if (intent.type === "competition" || (isDev && /start competition|end competition|next round|judge round/i.test(text))) {
    if (!isDev) { await send("Only Emmanuel.A can manage competitions."); return; }
    const t = text.toLowerCase();

    if (/start competition/i.test(text)) {
      const topic = text.replace(/start competition[:\s]*/i, "").trim() || "General Knowledge";
      const { announcement } = await startCompetition(jid, topic, pushName);
      await sock.sendMessage(replyJid, { text: announcement, mentions });
      await new Promise(r => setTimeout(r, 2000));
      const lumeoAns = await lumeoRespond(jid, sysPrompt);
      if (lumeoAns) await sock.sendMessage(replyJid, { text: "Lumeo AI responds:\n\n" + lumeoAns, mentions });
      await saveReply("[Competition started: " + topic + "]");
      return;
    }
    if (/end competition/i.test(text)) {
      const result = await endCompetition(jid);
      await sock.sendMessage(replyJid, { text: result, mentions });
      return;
    }
    if (/next round/i.test(text)) {
      const r = nextRound(jid);
      if (!r) { await send("No active competition. Start one: start competition [topic]"); return; }
      if (r.done) { await send("All rounds done! Say 'end competition' for final results."); return; }
      await sock.sendMessage(replyJid, { text: r.msg, mentions });
      await new Promise(r2 => setTimeout(r2, 2000));
      const lumeoAns2 = await lumeoRespond(jid, sysPrompt);
      if (lumeoAns2) await sock.sendMessage(replyJid, { text: "Lumeo AI responds:\n\n" + lumeoAns2, mentions });
      return;
    }
    if (/judge/i.test(text)) {
      const comp = getCompetition(jid);
      if (!comp) { await send("No active competition to judge."); return; }
      const metaAnswer = text.replace(/judge[:\s]*/i, "").trim() || "(Meta AI answer not provided)";
      const { judgment, roundSummary } = await judgeRound(jid, "(Lumeo's previous answer)", metaAnswer);
      await sock.sendMessage(replyJid, { text: "ROUND JUDGMENT:\n\n" + (judgment || "") + "\n\n" + (roundSummary || ""), mentions });
      return;
    }
    await send("Competition commands:\n- start competition [topic]\n- next round\n- judge [Meta AI answer]\n- end competition");
    return;
  }
  ...
  if (intent.type === "status_post") {
    ...
    const rawStatus = intent.prompt || text;

    if (/image.*status|status.*image|post.*image.*status|generate.*image.*status/i.test(rawStatus)) {
      await sock.sendMessage(replyJid, { text: "Generating image for status...", mentions });
      const imgDesc = rawStatus.replace(/generate|post|on.*status|status|image/gi, "").trim() || "EMEMZYVISUALS DIGITALS professional promotional";
      const imgBuf  = await generateImage(imgDesc);
      if (imgBuf) {
        const caption = await generateStatusContent("EMEMZYVISUALS DIGITALS");
        const ok = await postImageStatus(imgBuf, caption);
        await send(ok ? "Image posted to status!" : "Image status failed — no contacts loaded. Run 'list users' first.");
      } else { await send("Image generation failed."); }
      return;
    }

    if (/video.*status|status.*video|post.*video.*status|generate.*video.*status|market.*video/i.test(rawStatus)) {
      await sock.sendMessage(replyJid, { text: "Generating video for status...", mentions });
      try {
        const { generateVideo } = require("./lumeo_video");
        const vDesc = rawStatus.replace(/generate|post|on.*status|status|video|market/gi, "").trim() || "EMEMZYVISUALS DIGITALS services";
        const vr    = await generateVideo(vDesc);
        if (vr?.success && vr.buffer) {
          const caption = await generateStatusContent("EMEMZYVISUALS DIGITALS");
          const ok = await postVideoStatus(vr.buffer, caption);
          await send(ok ? "Video posted to status!" : "Video status failed.");
          if (vr.cleanup) vr.cleanup();
        } else { await send("Video generation failed."); }
      } catch (e) { await send("Video error: " + e.message); }
      return;
    }

    const statusText    = rawStatus.replace(/post.*status|update.*status|post status/gi, "").trim();
    const statusContent = await generateStatusContent(statusText || null);
    const ok = await postStatus(statusContent);
    await send(ok
      ? "Status posted!\n\n\"" + statusContent.slice(0, 100) + "...\""
      : "Status failed. Run 'list users' to check contacts are loaded.");
    return;
  }
