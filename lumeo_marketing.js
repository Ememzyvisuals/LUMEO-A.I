...
📧 ememzyvisualsdigitals@gmail.com | 📞 +234 904 711 5612`,
...
async function runPromotion(sock, params) {
  ...
  for (const jid of targets) {
    try {
      ...
      // Verify JID is on WhatsApp before sending (reduces failures)
      let isOnWA = true;
      try {
        const [result] = await sock.onWhatsApp(cleanJid.split("@")[0]).catch(() => [null]);
        if (result && !result.exists) { isOnWA = false; }
      } catch {}

      if (!isOnWA) {
        console.log(`[Marketing] ⚠️  ${cleanJid.split("@")[0]} not on WhatsApp`);
        failed++;
        continue;
      }

      await sock.sendMessage(cleanJid, { text: promoMsg });
      sent++;
      console.log(`[Marketing] ✅ Sent to ${cleanJid.split("@")[0]}`);
    } catch (e) {
      failed++;
      console.log(`[Marketing] ❌ ${jid.split("@")[0]}: ${e.message}`);
    }
    // 3-5 second delay between messages to avoid spam detection
...
From: Emmanuel.A, CEO — ememzyvisualsdigitals@gmail.com | +234 904 711 5612
